...
{
	struct pl2303_private *spriv = usb_get_serial_port_data(serial->port[0]);
	u8 request;
	int res;
	
	if (spriv->type == TYPE_HXN)
		...
		struct usb_serial_port *port, struct ktermios *old_termios)
{
	...
	if (priv->type == TYPE_HXN)
		pl2303_vendor_write(7, 0x07, serial);
	}
	...
	if (cflag & CRTSCTS) {
		...
		else if(priv->type == TYPE_HXN)
			pl2303_vendor_write(0x0A, 0xFA, serial);
		else 
			pl2303_vendor_write(0x0, 0x41, serial);
	} else {
		if (priv->type == TYPE_HXN)
			pl2303_vendor_write(0xA, 0xFF, serial);
		else 
...
static int pl2303_open(struct tty_struct *tty, struct usb_serial_port *port)
{
    ...
    if (priv->type == TYPE_HXN) pl2303_vendor_write(7, 0x07, serial);
